<?php
    ...
    $str .= "<div class='cap'>";
    $str.= "<a href='".base_url()."reportwithdrawal/viewReportWithdrawal/print/".urlencode(serialize($keys))."?StartDate=".$StartDate."&EndDate=".$EndDate."' target='/_blank' class='btn btn-icon icon-left btn-secondary' style='margin-left:10px;'><i class='fa fa-print'></i> print</a>";
    $str.= "<a href='".base_url()."reportwithdrawal/viewReportWithdrawal/export/".urlencode(serialize($keys))."?StartDate=".$StartDate."&EndDate=".$EndDate."' target='/_blank' class='btn btn-icon icon-left btn-success' style='margin-left:10px;'><i class='fa fa-file-excel'></i> export</a>";
    $str.= "<div class='cap' style='text-align: center; font-weight: bold;'> Report Withdrawal<br/ >Effective Date : ".date_format(date_create($StartDate),"d F Y")." - ".date_format(date_create($EndDate),"d F Y");
    $str .= "</div>";
    $str .= "</div>";
    $str .= "</div>";
    ...
    if(!empty($keys)){
      ...
      $tbl .= "<thead>";
      $tbl .= "<tr class='total'>";
      $tbl .= "<td>WithdrawalID</td>";
      $tbl .= "<td>WithdrawalDate</td>";
      $tbl .= "<td>AdminName</td>";
      ...
      foreach($keys as $value){ 
        ...
        if ($pco != $value->StudentID) {
          if ($pco != "") {
            $str .= "<tr class='total'>";
            $str .= "<td colspan='3'>Total Withdrawal</td>";
        ...
        $tot += $value->TotalWithdrawal;
        $str .= "<tr class='". $evenOdd ."'>";
        $str .= "<td nowrap title=\"Withdrawal\">". $value->WithdrawalID  ."</td>";
        $str .= "<td nowrap title=\"WithdrawalDate\">". date_format(date_create($value->WithdrawalDate),"d F Y") ."</td>";
