...
class Setupprofile extends CI_Controller {
    ...
    //   $res = $this->db->query("SELECT SetupprofileID FROM M_Setupprofile WHERE SetupprofileID = '".$stpid."'");
    //       if ($res->num_rows() == 0) {
						
    //         $this->db->query("INSERT INTO M_Setupprofile
				// 														( SetupTitle, SetupName, SetupDescription, SetupImageDasbor, SetupImage, setupLogo,
    //                                   IsActive, EntryBy, EntryDate, LastUpdateBy, LastUpdateDate ) 
				// 											VALUES 
				// 														( '".$stptl."', '".$stpnm."', '".$stpdc."',  '".$stpimg."', '','',
    //                                   'Y', '".$usernm."', '".$datetm."', '".$usernm."',  '".$datetm."')	
				// 										");
				// 		$msg = "Save";
    //       }
    ...
    //   $data['datashelf']    = $this->Bookshelf_model->ViewGetClass()->result();
